...
@router.post("", response_model=LocationDetail, dependencies=[Depends(require_admin)])
def create_location(payload: LocationCreateRequest):
    return location_service.create_location(payload.model_dump())


@router.put("/{project_id}", response_model=LocationDetail, dependencies=[Depends(require_admin)])
